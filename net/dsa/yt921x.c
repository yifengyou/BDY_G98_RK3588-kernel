...
/* ==========================================
 * 用于支持 YT9215 RGMII 的底层寄存器定义
 * ========================================== */
#ifndef YT921X_XMII_CTRL
#define YT921X_XMII_CTRL		0x80394
#endif

#ifndef YT921X_XMII_CTRL_PORTn
#define YT921X_XMII_CTRL_PORTn(port)	BIT(9 - (port)) /* 交换机外部级联口映射 */
#endif
...
static inline u32 dsa_bridge_ports(struct dsa_switch *ds, const struct net_device *bridge_dev)
{
	struct dsa_port *dp;
	u32 mask = 0;
...
static int yt921x_dsa_port_setup(struct dsa_switch *ds, int port)
{
	...
	if (dsa_is_cpu_port(ds, port)) {
		...
		if (dp && dp->conduit) {
			...
			/* 将期望特性 (wanted) 和当前生效特性 (features) 默认关闭 */
			dev->wanted_features &= ~csum_mask;
			dev->features        &= ~csum_mask;
...
static int yt921x_chip_setup_dsa(struct yt921x_priv *priv)
{
	...
	for (port = 0; port < YT921X_PORT_NUM; port++) {
		if (!(BIT(port) & priv->cpu_ports_mask))
			continue;

		if (yt921x_port_is_external(port)) {
			/* 关闭该 CPU 端口的 SERDES (SGMII) 控制 */
			res = yt921x_reg_clear_bits(priv, YT921X_SERDES_CTRL, YT921X_SERDES_CTRL_PORTn(port));
			if (res)
				return res;

			/* 开启该 CPU 端口的 XMII (RGMII) 控制 */
			res = yt921x_reg_set_bits(priv, YT921X_XMII_CTRL, YT921X_XMII_CTRL_PORTn(port));
			if (res)
				return res;
	...
	// 全局开启交换机的 LED 控制器并配置为并口（PARALLEL）模式 
	// 将寄存器 0xd0000 的 Bit 21 (LED_CTRL_EN) 设为 1，且保留系统通道数配置
	yt921x_reg_write(priv, 0xd0000, BIT(21) | (10 << 13));

	// 依次配置 Port 1 到 Port 4 物理网口的 LED 闪烁动作
	for (port = 1; port <= 4; port++) {
		/* 配置 LED0 控制 (0xd0004 + 4 * port)
		 * 动作包括：10M/100M/1000M 建立时常亮 (Bit 4,5,6) + 
		 * RX/TX 数据传输时闪烁 (Bit 9,10) + 活性指示 (Bit 13) + 禁用 Link Try (Bit 17)
		 * 对应数值：0x22670
		 */
		yt921x_reg_write(priv, 0xd0004 + 4 * port, 0x22670);

		/* 配置 LED1 控制 (0xd0040 + 4 * port)
		 * 动作包括：10M/100M/1000M 建立时常亮 (Bit 4,5,6) + RX/TX 数据传输时闪烁 (Bit 9,10) + 活性指示 (Bit 13)
		 * 对应数值：0x02670
		 * 0x00002670: 2Hz, 0x50002670: 4Hz, 0xa0002670: 8Hz, 0xf0002670: 16Hz
		 */
		yt921x_reg_write(priv, 0xd0040 + 4 * port, 0xa0002670); // 请注意高位改为了 0xf

		/* 配置 LED2 控制 (0xd0080 + 4 * port)
		 * 对应数值：0x02670
		 */
		yt921x_reg_write(priv, 0xd0080 + 4 * port, 0x02670);
	}
...
#include "yt_smi_mib_rw.c"
/* 全局引用计数，防止多芯片 probe 时重复创建 /proc 节点 */
static atomic_t yt921x_instance_count = ATOMIC_INIT(0);

static void yt921x_mdio_remove(struct mdio_device *mdiodev)
{
	struct yt921x_priv *priv = mdiodev_get_drvdata(mdiodev);
	
	if (priv == yt921x_units[0])
		yt921x_units[0] = NULL;
	else if (priv == yt921x_units[1])
		yt921x_units[1] = NULL;
...
static int yt921x_mdio_probe(struct mdio_device *mdiodev)
{
	...
	/* 自动根据总线名称区分 stmmac-0 (unit 0) 与 stmmac-1 (unit 1) */
	if (strstr(dev_name(&mdiodev->bus->dev), "stmmac-0") || strstr(mdiodev->bus->name, "stmmac-0"))
		yt921x_units[0] = priv;
	else if (strstr(dev_name(&mdiodev->bus->dev), "stmmac-1") || strstr(mdiodev->bus->name, "stmmac-1"))
		yt921x_units[1] = priv;

	if (atomic_inc_return(&yt921x_instance_count) == 1) {
		smi_mib_proc_test();
	}
